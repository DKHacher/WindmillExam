...
export class WindmillClient {
    ...
    getTelemetries(connectionId: string | undefined): Promise<RealtimeListenResponseOfListOfWindmillTelemetryDTO> {
        let url_ = this.baseUrl + "/telemetry-realtime?";
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetTelemetries(_response);
        });
    }

    protected processGetTelemetries(response: Response): Promise<RealtimeListenResponseOfListOfWindmillTelemetryDTO> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as RealtimeListenResponseOfListOfWindmillTelemetryDTO;
            return result200;
            });
        ...
        return Promise.resolve<RealtimeListenResponseOfListOfWindmillTelemetryDTO>(null as any);
    }
...
export interface RealtimeListenResponseOfListOfWindmillTelemetryDTO extends RealtimeListenResponse {
    data?: WindmillTelemetryDTO[] | undefined;
}

export interface WindmillTelemetryDTO {
    turbineId?: string;
    turbineName?: string;
    farmId?: string;
    timestamp?: string;
